...
public class VentanaPrincipal extends javax.swing.JFrame {
    ...
    public VentanaPrincipal() {
        ...
        resolucionOriginal = panelDeDibujo.getSize();
    ...
    private void actualizarSlider() {
        int valorSlider = Math.round(panelDeDibujo.escala * 10);
        jSliderZoom.setValue(valorSlider);
        panelDeDibujo.setBounds(0, 0, Math.round(resolucionOriginal.width * panelDeDibujo.escala), Math.round(resolucionOriginal.height * panelDeDibujo.escala));
        areaDibujoPanel.setBounds(0, 0, Math.round(resolucionOriginal.width * panelDeDibujo.escala), Math.round(resolucionOriginal.height * panelDeDibujo.escala));
        zoomValor.setText(new String(Math.round(panelDeDibujo.escala * 100) + "%"));
    }
    ...
    private void CursorBorrador() {
        ...
        Toolkit tk = Toolkit.getDefaultToolkit();
        Cursor cursorPincel = tk.createCustomCursor(Icono.getImage(), new Point(5, 30), "Cursor Borrador");
        setCursor(cursorPincel);
    }
    ...
    private void CursorGotero() {
        ...
        Toolkit tk = Toolkit.getDefaultToolkit();
        Cursor cursorPincel = tk.createCustomCursor(Icono.getImage(), new Point(3, 28), "Gotero");
        setCursor(cursorPincel);
    }
...
//    }   
    ...
    private void initComponents() {
        ...
        jPanelColorPrincipal.getAccessibleContext().setAccessibleName("");
    ...
    private void botonLupaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botonLupaActionPerformed
        // TODO add your handling code here:
        if (botonLupa.isSelected()) {
            panelDeDibujo.figuraSeleccionada = "Lupa";
            panelDeDibujo.addMouseListener(new MouseAdapter() {
                @Override
                public void mouseClicked(MouseEvent e) {
                    if (!panelDeDibujo.figuraSeleccionada.equals("Lupa")) {
                        panelDeDibujo.removeMouseListener(this);
                    } else {
    ...
    private void botonGoteroActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botonGoteroActionPerformed
        ...
        if (botonGotero.isSelected()) {
            panelDeDibujo.figuraSeleccionada = "Gotero";
            panelDeDibujo.addMouseListener(new MouseAdapter() {
                @Override
                public void mousePressed(MouseEvent e) {
                    if (!panelDeDibujo.figuraSeleccionada.equals("Gotero")) {
                        panelDeDibujo.removeMouseListener(this);
                    } else {
                        Robot robot;
                        try {
                            ...
                            Color color = robot.getPixelColor(e.getLocationOnScreen().x, e.getLocationOnScreen().y);
                            jPanelColorPrincipal.setBackground(color);
                            System.err.println(color);
                            System.err.println(e.getLocationOnScreen().x + " " + e.getLocationOnScreen().y);
    ...
    private void jCheckBoxSimétricoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jCheckBoxSimétricoActionPerformed
        if (panelDeDibujo.figuraSimetrica) {
            panelDeDibujo.figuraSimetrica = false;
        } else if (!panelDeDibujo.figuraSimetrica) {
            panelDeDibujo.figuraSimetrica = true;
        }
    ...
    public javax.swing.JPanel jPanelColorPrincipal;
    public javax.swing.JPanel jPanelColorSecundario;
